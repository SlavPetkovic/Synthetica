...
class Application(customtkinter.CTk):
    def __init__(self):
        ...
        self.image_generation_complete = True
    ...
    def init_ui(self):
        ...
        try:
            # Inside the init_ui method, after creating the response_canvas
            self.download_button = customtkinter.CTkButton(self, text="Download Image", command=self.download_image)
            self.download_button.grid(row=4, column=1, columnspan=3, padx=20, pady=10, sticky="ew")
        except Exception as e:
            logging.exception("Failed to create question entry and submit button.")
    ...
    def update_canvas_message(self, message, count=0):
        ...
        dots = "." * (count % 4)  # Change the number of dots every second
        final_message = message + dots
        self.response_canvas.delete("all")  # Clear the canvas
        self.response_canvas.create_text(self.response_canvas.winfo_width() // 2,
                                         self.response_canvas.winfo_height() // 2,
                                         text=final_message, fill="white", font=("Helvetica", 40))
        count += 1
        self.after(1000, self.update_canvas_message, message, count)

    def generate_and_display_dalle_image(self, question):
        # Set image generation flag to False and start the updating message
        self.image_generation_complete = False
        self.update_canvas_message("Generating Image")

        def after_image_generation():
            try:
                ...
                self.response_canvas.photo_image = photo_image  # Keep a reference to the image
            except Exception as e:
                logging.error(f"Error generating/displaying DALL-E image: {e}")
            finally:
                self.image_generation_complete = True  # Set this to True when image generation is complete or an exception occurs
                self.update_canvas_message("")  # Clear the generating message

        # Run the image generation in a separate thread
        threading.Thread(target=after_image_generation).start()
    ...
    def download_image(self):
        if hasattr(self, 'original_pil_image'):  # Check if there is a PIL image saved
            filepath = filedialog.asksaveasfilename(defaultextension=".png",
                                                    filetypes=[("PNG files", "*.png"), ("All files", "*.*")])
            if filepath:
                try:
                    self.original_pil_image.save(filepath)  # Save the original PIL image
                    messagebox.showinfo("Download successful", f"Image successfully saved to {filepath}")
                except Exception as e:
